...
class Application extends BaseApplication
{
    ...
    public function middleware(MiddlewareQueue $middlewareQueue): MiddlewareQueue
    {
        ...
        $csrfMiddleware->skipCheckCallback(static function ($request): bool {
            $path = '/' . ltrim($request->getPath(), '/');
            $acceptsJson = str_contains($request->getHeaderLine('Accept'), 'application/json');
            $sendsJson = str_contains($request->getHeaderLine('Content-Type'), 'application/json');

            return str_starts_with($path, '/api/') || ($acceptsJson && $sendsJson);
        });
        ...
        $middlewareQueue
            ...
            // Allow the SPA and mobile-friendly web client to talk to the API
            // during local development and in split deployments.
            ->add(new ApiCorsMiddleware())

            // Lightweight bearer token auth for the API routes.
            ->add(new ApiAuthenticationMiddleware(Security::getSalt()))
